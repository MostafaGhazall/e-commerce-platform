export function pickVariantImage(item: {
  color?: string | null;
  colorName?: string | null;
  product: {
    colors: { name: string; value: string; images: { url: string }[] }[];
    images: { url: string }[];
  };
}): string {
  // 1️⃣ match by hex‐value first
  if (item.color) {
    const matchByValue = item.product.colors.find(
      c => c.value.toLowerCase() === item.color!.toLowerCase()
    );
  ...
  if (item.colorName) {
    const matchByName = item.product.colors.find(
